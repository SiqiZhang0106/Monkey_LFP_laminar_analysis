...
for t_idx = 1:n_trials
    ...
    if trial.reach_hand==2
        ...
        %     target_onset_win=[target_on_center+target_onset_woi(1)*1000:target_on_center+target_onset_woi(2)*1000];
        %     target_onset_times=times(target_onset_win)-times(target_on_center);
        %     lfp_target_onset(t_idx,:,:) = Xsignal(:,target_onset_win);
...
for i=1:size(csdnew,1)
    csd_reach_onset(i,:) = (csdnew(i,:) - nanmean(csd_baseline(i,:),2));%/csdvar(i);
end
